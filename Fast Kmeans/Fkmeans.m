function [centers,mincenter,mindist,q2,quality] = Fkmeans(data,initcenters,method)
...
if max(size(initcenters)) == 1
    k = initcenters;
    [centers, mincenter, mindist, lower, computed] = Fanchors(mean(data),k,data);
    total = computed;
    skipestep = 1;
else 
    centers = initcenters;
...
while nchanged > 0
    ...
    if method == 0 && ~skipestep
        for i = 1:n
            for j = 1:k
                distmat(i,j) = Fcalcdist(data(i,:),centers(j,:));
            end
        end
    ...
    elseif (method == 1 || (method == 2 && iteration == 0)) && ~skipestep
        ...
        lower = zeros(n,k);
        for j = 1:k
           jdist = Fcalcdist(data,centers(j,:));
           lower(:,j) = jdist;
...
% redo may be empty, but we don't need to check this
            ...
            for jj = unique(c)'
                rp = redo(c == jj);
                udist = Fcalcdist(data(rp,:),centers(jj,:));
                lower(rp,jj) = udist;
                mindist(rp) = udist;
            end
            recalculated(redo) = 1;
            ...
            jdist = Fcalcdist(data(track5,:),centers(j,:));
...
% it's correct to have mean(data(track,:),1) but this can make answer worse!
    ...
    if method == 2
        for j = diffj
            offset = Fcalcdist(centers(j,:),oldcenters(j,:));
            computed = computed + 1;
...
udist = Fcalcdist(data,centers(mincenter,:));
quality = mean(udist);
q2 = mean(udist.^2);
