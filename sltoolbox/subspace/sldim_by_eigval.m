...
if ischar(sch)
    ...
    switch sch
        ...
        case 'energy'
            fh = @dim_by_energy;
        case {'Everrit','everrit'}
            fh = @dim_by_everrit;
        case {'profileML','ProfileML','profileml','Profileml'}
            fh = @dim_by_profileML;
        case 'number'
            fh = @dim_by_number;
        otherwise
...
function k = dim_by_rank(evals, r)
...
function k = dim_by_everrit(evals, r)

k = sum(evals > 0.7/length(evals) * sum(evals));

function k = dim_by_profileML(evals, r)

k = Select_Dimension_withLikelihood(evals);

function k = dim_by_number(evals, r)
