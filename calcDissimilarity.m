function disMat = calcDissimilarity(i,j,disDim2,Pat)
global X;
...
%% Calculate dissimilarity by Hausdorff distance

    [r,c] = find(A);
    pointA = [r c];
    [r,c] = find(B);
    pointB = [r c];
    disMat = sqrt(max( compute_dist(pointA,pointB), compute_dist(pointB,pointA) ));
